...
namespace structures 
{
	...
	Array<T>& Array<T>::operator=(const Array<T>& other)
	{
		...
		if (size_ != other.size_) {
			throw std::exception("No same size of array");
	...
	{
		//copy funkciu z vektora a range checky na indexi , ak length je 0 nic sa nerobi
		if (length != 0) {
			DSRoutines::rangeCheckExcept(srcStartIndex,src.size_,"Invalid index");
			DSRoutines::rangeCheckExcept(srcStartIndex + length - 1, src.size_, "Invalid index");
			DSRoutines::rangeCheckExcept(destStartIndex, dest.size_, "Invalid index");
			DSRoutines::rangeCheckExcept(srcStartIndex + length - 1, dest.size_, "Invalidindex");
