...
namespace structures
{
	...
	inline LinkedList<T>& LinkedList<T>::operator=(const LinkedList<T>& other)
	{
		...
		{
			clear();
			LinkedListItem<T> *currentOther = other.first_;
			while (currentOther != nullptr)
			{
				add(currentOther->accessData());
	...
	{
		DSRoutines::rangeCheckExcept(index, size_, "Invalid index");
		LinkedListItem<T> *remItem = nullptr;
		if (index == size_ - 1) {
			if (first_ == last_) {
				remItem = first_;
				first_ = nullptr;
			...
			remItem = first_;
			first_ = remItem->getNext();
			remItem->setNext(nullptr);
			size_--;
			return remItem->accessData();
		} else {
			LinkedListItem<T> *prev = getItemAtIndex(index - 1);
			remItem = prev->getNext();
			prev->setNext(remItem->getNext());
			remItem->setNext(nullptr);
			size_--;
			return remItem->accessData();
			}
