...
namespace structures
{
	...
	{
		DSRoutines::rangeCheckExcept(index,size_,"Invalid index");//potrebne skontrolovat lebo idex v array kontrola velkosti, size v arrayliste pocet kolko mam vlozenych v arrayliste
		return (*array_)[index]; // potrebne dereferencovat
	}
	...
	inline void ArrayList<T>::add(const T & data)
	{
		...
		(*array_)[size_] = data; // prida prvok, potrebne dereferencovat
		size_++;
	}

	template<typename T>
	inline void ArrayList<T>::insert(const T & data, const int index) //to iste ako remove,
	{
	...
	inline T ArrayList<T>::removeAt(const int index)
	{
		...
		if (size_ < (array_->size() / 4)) {
			Array<T> *a = new Array<T>(array_->size() / 2);
			Array<T>::copy(*array_, 0, *a, 0, size_ - 1);
			*array_ = *a;
			delete a;
		} 
		size_--;
	...
	template<typename T>
	inline void ArrayList<T>::clear() //zmensovanie pola ak je to vyhodne, zmensovanie clear() , removeAt(),... defaultne na 4
	{
		size_ = 0;
		delete array_;
		array_ = new Array<T>(4);
